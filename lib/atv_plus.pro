...
;        'f': osirisclean_cleanpix, state.coord
        ...
        'k':  begin ; write keyword value, by default writes DATAQUAL
               atv_update_keyword_event,{id:0, top:1, handler:0, select:1}
            end
        'o': begin ; overplot position
               atv_overplot_event,{id:0, top:1, handler:0, select:1}
            end
...
pro atv_apphot_toside, xypos
   
common atv_state

atv_apphot
if n_elements(xypos) eq 2 then begin
   state.centerboxsize=0
   
   state.centerpos[0] = xypos[0]
   state.centerpos[1] = xypos[1]
   
   state.cursorpos[0] = xypos[0]
   state.cursorpos[1] = xypos[1]
endif

if (xregistered('atv_apphot', /noshow)) then begin
   widget_control, state.showradplot_id, TLB_SET_XOFFSET=530
   widget_control, state.showradplot_id, get_value=val
   case val of
       'Show radial profile': begin
           ysize = 350 < (state.screen_ysize - 350)
    ...
    endcase
    atv_apphot_refresh
   ;atv_apphot_event,{id:state.showradplot_id, top:0L, handler:0L, select:1}
endif
...
pro atv_update_keyword_event, event
   
   common atv_state 
   common atv_change_key, change_key
   
   if not file_test(state.imagename) then begin
      print, 'FILE does not exist: '+state.imagename
      return
   endif
   
   case event.id of
      0: begin
            if (not (xregistered('atv_update_keyword', /noshow))) then begin
               top=widget_base(/column, /base_align_center, group_leader = state.base_id)
               title=widget_label(top,value="Select the name of the keyword to be changed as well as its value and type.")
               
               defkeyname = 'DATAQUAL'
               im = readfits(state.imagename,h,/silent)
               defkeyval = '['+strtrim(sxpar(h,defkeyname, count=ncount),2)+']'
               
               if ncount eq 0 then begin
                  if defkeyname eq 'DATAQUAL' then defkeyval='BAD' else defkeyval = ''
               endif
               
               namebase=widget_base(top,/column)
                  labelname=widget_label(namebase,value='Name')
                  keyname=widget_text(namebase,value=defkeyname,/editable,/no_newline,xsize=50)
               valuebase=widget_base(top,/column)
                  labelvalue=widget_label(valuebase,value='Value')
                  keyval=widget_text(valuebase,value=defkeyval,/editable,/no_newline,xsize=50)
               keytype=widget_droplist(top,value=['String','Float','Double','Integer'],title='Type')
               commentbase=widget_base(top,/column)
                  labelcomment=widget_label(commentbase,value='History Comment')
                  keycomment=widget_text(commentbase,value='',/editable,/no_newline,xsize=100)
               
               butbase=widget_base(top,/row)
               butok = widget_button(butbase,value='Apply All')
               buthistory = widget_button(butbase,value='Only History')
               butcancel = widget_button(butbase,value='Close')
               ...
               change_key={ top: top, $
                            ...
                            comment : keycomment, $
                            butok: butok, $
                            buthistory: buthistory, $
                            butcancel: butcancel}
               
               widget_control, top, /realize
               xmanager, 'atv_update_keyword', top
            endif
         end
      change_key.name: begin
                        if event.type eq 0 then begin
                           widget_control,change_key.name,get_value=getaux
                           im = readfits(state.imagename,h,/silent)
                           defkeyval =sxpar(h,getaux[0], count=ncount)
                           if (ncount gt 0) then begin
                              widget_control,change_key.value,set_value='['+strtrim(defkeyval,2)+']'
                              tp = size(defkeyval,/type)
                              if tp eq 2 or tp eq 1 then widget_control,change_key.type, SET_DROPLIST_SELECT=3
                              if tp eq 3 or tp eq 4 then widget_control,change_key.type, SET_DROPLIST_SELECT=1
                              if tp eq 5 then widget_control,change_key.type, SET_DROPLIST_SELECT=2
                              if tp gt 5 then widget_control,change_key.type, SET_DROPLIST_SELECT=0
                           endif else begin
                              widget_control,change_key.type, SET_DROPLIST_SELECT=0
                           endelse
                        endif
                       end
      change_key.butok: begin
                        
                        ; Read values to be changed
                        widget_control,change_key.name,get_value=getaux
                        keyname=getaux[0]
                        widget_control,change_key.value,get_value=getaux
                        keyvalue=strjoin(strsplit(getaux[0],'[]',/extract))
                        widget_control,change_key.type,get_value=getaux
                        id = widget_info(change_key.type, /DROPLIST_SELECT)
                        keytype=getaux[id]
                        widget_control,change_key.comment,get_value=getaux
                        keycomment=getaux[0]
                        
                        if (keytype eq 'Float') then keyvalue = float(keyvalue)
                        if (keytype eq 'Double') then keyvalue = double(keyvalue)
                        if (keytype eq 'Integer') then keyvalue = fix(keyvalue)
                        
                        if strupcase(keyname) eq 'DATAQUAL' then hcomment='Data Quality flag' else hcomment=''
                        
                        ; If header is initialized in state, then update it
                        if state.head_ptr ne ptr_new() then begin
                           sxaddpar,*(state.head_ptr),keyname,keyvalue, hcomment
                           print,'   ---> file update: '+state.imagename
                           print,'Keyword '+keyname+' updated to '+strtrim(keyvalue,2)+' of type '+strtrim(keytype,2)
                           if (keycomment ne '') then print,'     with history comment: '+keycomment  
                           if (keycomment ne '') then sxaddpar,*(state.head_ptr),'HISTORY',keyname+'->'+strtrim(keyvalue,2)+': '+keycomment
                        endif
                        
                        ; If filename is initialized in state, then update it
                        if file_test(state.imagename) then begin
                           im = readfits(state.imagename,h,/silent)
                           
                           ;sxdelpar,h,keyname
                           sxaddpar,h,keyname,keyvalue, hcomment
                           if (keycomment ne '') then sxaddpar,h,'HISTORY',keyname+'->'+strtrim(keyvalue,2)+': '+keycomment
                           print,'   ---> file update: '+state.imagename
                           print,'Keyword '+keyname+' updated to '+strtrim(keyvalue,2)+' of type '+strtrim(keytype,2)
                           if (keycomment ne '') then print,'     with history comment: '+keycomment  
                           writefits,state.imagename,im,h
                        endif
                        
                        widget_control, event.top, /destroy
                  end
      change_key.buthistory: begin
                        widget_control,change_key.comment,get_value=getaux
                        keycomment=getaux[0]
                        if (keycomment ne '') then sxaddpar,h,'HISTORY','DATAQUAL: '+keycomment
                        
                        ; If header is initialized in state, then update it
                        if state.head_ptr ne ptr_new() then begin
                           sxaddpar,*(state.head_ptr),keyname,keyvalue, hcomment
                           print,'   ---> file update: '+state.imagename
                           print,'     with history comment: '+keycomment  
                           sxaddpar,*(state.head_ptr),'HISTORY',keycomment
                        endif
                        ...
                        if file_test(state.imagename) then begin
                           ...
                           print,'   ---> file update: '+state.imagename
                           if (keycomment ne '') then print,'     with history comment: '+keycomment
                           sxaddpar,h,'HISTORY',keycomment 
                           writefits,state.imagename,im,h
                        endif
                        
                        widget_control, event.top, /destroy
                  end
      change_key.butcancel: begin
                     widget_control, event.top, /destroy
                  end
      else:
   endcase
...
pro atv_overplot
   ...
   print, 'Will ask for a position and overplot that position'
